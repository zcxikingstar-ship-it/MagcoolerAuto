...
- 直接读取与 Macs Fan Control「CPU Core Average」对应的 Apple SMC 核心温度，不要求 Macs Fan Control 常驻。
- 通过蓝牙控制 Redmagic Magcooler 的开关、低中高三档和灯效。
- 低档、中档、高档和关闭温度都可以直接填写，并自动保存。
- 切档确认、低温关闭等待和常规采样间隔都可以自行设置。
...
我先研究了 Macs Fan Control，确认 Mac 上确实能稳定拿到 CPU 传感器数据。但两个应用硬绑在一起并不优雅，所以最终选择直接读取 Apple SMC，并对齐它的「CPU Core Average」传感器组合。这样既能参考同一组核心温度，也少一个“到底是谁没启动”的排查现场。
...
自动控制也不是简单写几个 `if`：温度会瞬间跳动，蓝牙会断，散热器可能根本不在房间里。于是又陆续加上了切档确认、低温延迟关闭、定时采样、断线恢复，以及最后这个非常现实的“别再搜了”开关。后来这些温度和时间也全部做成了可编辑设置，不必为了改一个数字重新编译应用。
...
默认每 30 秒进行一次常规采样；达到切档阈值时，会在 10 秒后增加一次确认采样，兼顾日常开销和响应速度。55–60°C 区间保持当前状态，避免在临界温度附近反复启停。四个温度阈值、切档确认时间、低温关闭等待时间和采样间隔都可以在界面中修改，重启后仍会保留。

## 使用方法

1. 打开 `Magcooler 控制器.app`。
2. 按需要填写温度阈值和三个时间参数，点击对应的“保存”。
3. 选择“自动”模式。
4. 散热器在附近并已开机时，打开“蓝牙搜索”。
5. 首次连接按 macOS 提示允许蓝牙访问。
